...
namespace HiFi
{
    namespace Platform
    {
        public class HiFi_PlatformInitializer : MonoBehaviour
        {
            ...
            private bool avatarFinder = false, deviceLoaded, needLoadAttempt;
            ...
            private void Awake()
            {
                ...
                if (XREnabled)
                {
                    ...
                    AvatarAwake.avatarAwake += SetControllers;
                }
            }
            ...
            private void SetupXREnvironment()
            {
                ...
                /// Get Tracking Type
                if (XRDevice.SetTrackingSpaceType(targetTrackingSpace))
                    HiFi_Utilities.DebugText($"Successfully set TrackingSpaceType to ({targetTrackingSpace.ToString()})");
                else
                    HiFi_Utilities.DebugText("Failed to set TrackingSpaceType");
                trackingSpaceType = XRDevice.GetTrackingSpaceType(); HiFi_Platform.instance.trackingSpaceType = trackingSpaceType;
                trackingOriginMode = XRDevice.trackingOriginMode; HiFi_Platform.instance.trackingOriginMode = trackingOriginMode;
