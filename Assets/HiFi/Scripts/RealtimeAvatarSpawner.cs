...
namespace Normal.Realtime {
    ...
    public class RealtimeAvatarSpawner : MonoBehaviour {
...
#pragma warning restore 0649
        ...
        public delegate void AvatarCreatedDestroyed(RealtimeAvatarSpawner avatarSpawner, RealtimeAvatar avatar, bool isLocalAvatar);
        ...
        void DidConnectToRoom(Realtime room) {
            if (!gameObject.activeInHierarchy || !enabled || !XRSettings.enabled)
                return;
        ...
        public void CreateAvatarIfNeeded() {
            if (!_realtime.connected) {
                Debug.LogError("RealtimeAvatarSpawner: Unable to create avatar. Realtime is not connected to a room.");
                return;
            }
            ...
            if (avatarGameObject == null) {
                Debug.LogError("RealtimeAvatarSpawner: Failed to instantiate RealtimeAvatar prefab for the local player.");
                return;
            }

            localAvatar = avatarGameObject.GetComponent<RealtimeAvatar>();
            if (avatarGameObject == null) {
                Debug.LogError("RealtimeAvatarSpawner: Successfully instantiated avatar prefab, but could not find the RealtimeAvatar component.");
                return;
            }
